...
%                 f = waitbar(0, 'Initialize GPU! Waiting...');
%                 pause(.5)
                gpuDevice();
%                 waitbar(1, f, 'Done!');
%                 pause(.5)
...
for k = 1:length(features)
    if features{k}.is_cnn
        ...
        feature_info.channel_selection_rate_block{k} = repmat(features{k}.fparams.channel_selection_rate,numel(features{k}.fparams.output_var),1);
        ...
        feature_info.channel_selection_rate_block{k} = features{k}.fparams.channel_selection_rate;
