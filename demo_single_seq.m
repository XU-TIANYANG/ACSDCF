%% Demo : Adaptive Channel Selection for Robust Visual Tracking with Discriminative Correlation Filters (ACSDCF)
...
base_path  = '/vol/vssp/cvpnobackup/facer2vm/Zhenhua/tmparea/Tianyang/dataset_OTB'; %benchmark path
video = choose_video(base_path);
video_path = [base_path '/' video];
[seq, gt_boxes] = load_video(video_path,video);

% Run tracker
%results = run_ACSDCF(seq); % deep and hand-crafted features are used
results = run_ACSDCF_HC(seq); % only hand-crafted features are used
